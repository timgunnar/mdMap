...
mdMap 把搜索分成两步：**缩小范围** + **阅读摘要**。两步配合，精准定位到唯一一份文档。

```
Step 1: --type 缩小范围（强信号，确定性）
        你知道自己在干什么 → 你知道要找什么类型的文档
        "发布项目" → 先看约束规则 → --type rule
        "查历史资料" → 先看参考资料 → --type resource

Step 2: --search 语义检索（BM25 打分排序）
        在缩小后的集合里搜 → 结果通常只有 2-5 条
        读摘要 → 你自己的语义理解能力判断哪条对
```

**核心原则：永远加上 `--type`。** 不加 type 回到全文搜索，噪音回到几十条。加了 type，结果集本身就很小——规则文档本来就少，参考资料同理。然后你的语义理解能力在 2-5 行摘要里做最终判断，精准度远高于任何算法。

```bash
# 用户说："我要发布 project-a"
# 你判断：这是要遵守发布规则 → --type rule
mdmap find --search "project a publish" --type rule
# 0.86  [rule]  project_a_publish.md  — 发布 project-a 的规则：GitHub releases，vX.Y.Z 标签
# 0.42  [rule]  project_b_publish.md  — 发布 project-b 的规则：npm publish

# 两行结果，第一个摘要对上了 → 打开 project_a_publish.md。完成。
```

如果结果里没有完全匹配的摘要，换一两个关键词再搜（如 `--search "project-a release" --type rule`），而不是去打开所有结果——摘要已经告诉你每篇文档的核心内容了。
