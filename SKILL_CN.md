...
**把 `--search` 作为你的首选查询方法。** 它一次性地在全部文本字段（标题、摘要、定位、触发器、更新触发、退役触发、标签）中做子串匹配。直接把用户的意图扔进去就行：

```bash
# ✅ 首选方法——一次搜全部字段
mdmap find --search "发布"
mdmap find --search "帮我把这个发布到 GitHub"
mdmap find --search "push code"
```

你给 `--search` 的文本越多，里面可供匹配的子串就越多。用户说"我要把代码推上去发布到 GitHub"，这句话里就包含"代码"、"发布"、"GitHub"——任意子串在索引的某个字段里命中，文档就会被返回。**你不需要自己提取关键词，也不需要换着词重试。**
