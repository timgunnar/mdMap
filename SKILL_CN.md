...
`--trigger`、`--maintains`、`--retires` 使用的是**子串匹配**，不是语义搜索。它不会理解同义词或换一种说法。举个例子：用户说"推送github"、"帮我发布一下"、"把代码推上去"，这些表达方式各不相同，但都是在说同一件事。如果触发词里只写了"releasing to GitHub"，那"推送github"就搜不到。

**你的工作是把用户的意图翻译成短关键词。** 当用户说"帮我把这个发布到 GitHub"或"我要推代码"时：

```bash
# ✅ 正确做法——提取核心词，多试一两个变体
mdmap find --trigger "发布"
mdmap find --trigger "publish"
mdmap find --trigger "github"

# ❌ 错误做法——把用户一整句话当查询
mdmap find --trigger "帮我把这个发布到 GitHub"
```

第一次搜不到就换一两个同义词再试。索引里的触发词应该已经被填索引的 LLM 写成了多样化的关键词——所以短词、高频词通常能匹配上。
